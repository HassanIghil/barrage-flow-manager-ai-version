import React, { useState, useEffect } from "react";
import { AlertTriangle, X, ChevronRight, Bell } from "lucide-react";
import api from "../api/axios";
import { Link } from "react-router-dom";

export default function AlertBanner() {
  const [alerts, setAlerts] = useState([]);
  const [isVisible, setIsVisible] = useState(true);
  const [loading, setLoading] = useState(true);

  const fetchAlerts = async () => {
    try {
      // Appel à ton endpoint FastAPI /api/alerts/recent
      const response = await api.get("/alerts/recent");
      
      // On prend exactement les 5 premières (les plus récentes via l'ORDER BY du backend)
      const latestFive = response.data.slice(0, 5);
      setAlerts(latestFive);
    } catch (err) {
      console.error("Erreur alertes:", err);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchAlerts();
    // Rafraîchissement automatique toutes les minutes
    const interval = setInterval(fetchAlerts, 60000);
    return () => clearInterval(interval);
  }, []);

  // On ne rend rien si : fermé, chargement, ou aucune alerte en base
  if (!isVisible || loading || alerts.length === 0) return null;

  // Si au moins une des 5 alertes est de type 'niveau_critique'
  const hasCritical = alerts.some(a => a.type_alerte === "niveau_critique");

  return (
    <div className={`mb-8 animate-in fade-in slide-in-from-top duration-700 rounded-[24px] border p-5 shadow-sm transition-all ${
      hasCritical 
        ? "border-ember/30 bg-ember/5 text-ember" 
        : "border-amber-200 bg-amber-50 text-amber-800"
    }`}>
      <div className="flex items-start gap-5">
        {/* Icône avec animation d'alerte si critique */}
        <div className={`shrink-0 rounded-2xl p-3 ${
          hasCritical ? "bg-ember text-white animate-pulse" : "bg-amber-500 text-white"
        }`}>
          {hasCritical ? <AlertTriangle size={22} /> : <Bell size={22} />}
        </div>
        
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-sm font-black uppercase tracking-[0.15em]">
              {hasCritical ? "Alertes Système Critiques" : "Flux d'alertes récentes"}
            </h3>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
               hasCritical ? "bg-ember text-white" : "bg-amber-200 text-amber-900"
            }`}>
              {alerts.length} Messages
            </span>
          </div>
          
          <div className="grid gap-2">
            {alerts.map((alert) => (
              <div 
                key={alert.id_alerte} 
                className={`flex items-center gap-3 text-sm font-medium p-2 rounded-xl transition-colors ${
                  alert.type_alerte === 'niveau_critique' ? 'bg-ember/10' : 'hover:bg-black/5'
                }`}
              >
                <span className="opacity-50 text-xs font-mono shrink-0">
                  {new Date(alert.date_alerte).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
                <p className="line-clamp-1">{alert.message}</p>
                {alert.type_alerte === 'niveau_critique' && (
                  <span className="ml-auto flex h-2 w-2 rounded-full bg-ember animate-ping"></span>
                )}
              </div>
            ))}
          </div>

          <div className="mt-4 pt-3 border-t border-current/10">
            <Link 
              to="/alerts" 
              className="inline-flex items-center gap-1 text-xs font-bold uppercase hover:underline"
            >
              Gérer toutes les alertes dans l'historique <ChevronRight size={14} />
            </Link>
          </div>
        </div>

        <button 
          onClick={() => setIsVisible(false)}
          className="shrink-0 rounded-xl p-2 transition hover:bg-black/10"
        >
          <X size={20} className="opacity-50" />
        </button>
      </div>
    </div>
  );
}